...
-- Feed-scoped stage completion (ingest, patterns, bus_evidence, gtfs_bus_way_evidence, …)
...
-- OSM import-run scoped stages (import sub-stages, segment_turns, …)
...
-- Per-pattern GTFS content signature (representative trip/shape/stop_times)
...
-- Pattern → OSM map-match stage status
...
-- Pattern edge backfill (Valhalla trace legs) stage status
...
-- Logical OSM dataset identity (PBF content + import argv); independent of run id
ALTER TABLE osm_import_runs
    ADD COLUMN IF NOT EXISTS osm_dataset_fingerprint TEXT;

CREATE INDEX IF NOT EXISTS idx_osm_import_runs_dataset_fp
    ON osm_import_runs (osm_dataset_fingerprint)
    WHERE status IN ('success', 'verify_only');

-- Legal anchors: pipeline skip state (distinct from business outcome column)
...
DO $legal_anchor_pipeline_outcome$
BEGIN
    IF NOT EXISTS (
