...
- **Legacy** (`/api/v1/detours/by-area`, `detour_service.py`, `detour_graph.py`): GTFS pattern graph + optional Valhalla hybrid; caches in `detour_by_area_cache`.
...
| `bus_feasibility_evaluator.py` | Hard bans, penalties, U-turn/backtrack/sharp-turn detection. |
...
7. **BusFeasibilityEvaluator** + **DetourRanker** → best candidate or `no-safe-detour-found`.
